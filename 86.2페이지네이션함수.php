...
//출력 페이지수 << 1 2 3 4 5>>
    ...
    #첫번째 페이지
    $return_str = "<a href='".$_SERVER['PHP_SELF']."?page=1'>첫번째 페이지</a>";
    ...
    if($prev_page > 1 ){
    $return_str .= "<a href='".$_SERVER['PHP_SELF']."?page={$prev_page}'> 이전 페이지 버튼 </a>";
    }
    ...
    */
    ...
    for($i=$start_page; $i <= $end_page; $i++){ if($page==$i){ $return_str .=$i.' ';
        
}else{
    $return_str.= "<a href=' ".$_SERVER['PHP_SELF']."?page={$i}'>{$i}</a> ";
}
}
...
if($next_page <= $total_page ){
    $return_str .= "<a href=' ".$_SERVER['PHP_SELF']."?page= {$next_page} '>  다음 페이지 버튼  </a> ";
}

#마지막 페이지
$return_str .= "<a href=' ".$_SERVER['PHP_SELF']."?page={$total_page}'>마지막 페이지</a>";
